...
export TXS_VERSION=$(sed -ne 's/^#define TXSVERSION "\(.*\)".*$/\1/p' src/utilsVersion.h)
...
if [ "${OS_NAME}" = "Linux" ]; then
	export RELEASE_DATE=$(date -u +"%Y-%m-%dT%H:%M:%S%z" --date="${GIT_DATE}")
...
export VERSION_NAME="${TXS_VERSION}-${DATE_HASH}-git_${GIT_HASH}"
