...
void QEditor::insertFromMimeData(const QMimeData *d)
{
	if ( d && m_cursor.isValid() && !isReadOnly() )
	{
		...
		if ( d->hasFormat("text/column-selection") )
		{
			...
			// check whether it is a spreadsheet
            if (d->hasFormat("text/plain") && d->hasFormat("application/x-openoffice-dif;windows_formatname=\"DIF\""))
			{
                txt = d->text();
				txt = txt.replace("\t", " & ");
                txt = txt.replace("\n", " \\\\\n");
			}
			else if ( d->hasFormat("text/plain") )
				txt = d->text();
