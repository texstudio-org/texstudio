...
void Texstudio::setupDockWidgets()
{
	...
	int k = viewNr;
	int i;
	for (i = 0; i < leftPanel->widgetCount(); i++)  {
		if (!leftPanel->hiddenWidgets().split("|").contains(leftPanel->widget(i)->property("id").toString())) {
			if (k == 0) {
	...
	if (i >= leftPanel->widgetCount()) // all tools are hidden
