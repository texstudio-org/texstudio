...
bool BuildManager::m_interpretCommandDefinitionInMagicComment = true;
		...
		if (!subcmd.startsWith(TXS_CMD_PREFIX))  {
            ...
            if (match.hasMatch()) {
                ...
                if (cmd.isEmpty()) {
                    ...
                    else if (cmdName != "pre-compile") qDebug() << tr("Command %1 not defined").arg(subcmd); //pre-compile is expected
